...
public class MainActivity extends Activity implements CameraManager.CameraListener, View.OnClickListener {
    ...
    /* 人脸识别Group */
    private static final String group_name = "facepaa";
            ...
            public void run() {
                while (true && !isFinishing()) {
                    if (FacePassHandler.isAvailable()) {
                        ...
                        livenessModel = FacePassModel.initModel(getApplicationContext().getAssets(), "panorama.facepass.0813_0805_cpu_3models_1core.180813_0805_cpu_3models_1core.bin");
                        searchModel = FacePassModel.initModel(getApplicationContext().getAssets(), "feat.facepass.4comps.inp96.inu1000_ft.bin");
                        ...
                        try {
                            /* 填入所需要的配置 */
                            Log.d(DEBUG_TAG, "rotation " + rotation);
    ...
    private class FeedFrameThread extends Thread {
        ...
        public void run() {
            while (!isInterrupt) {
                ...
                if (mFacePassHandler == null) {
                    Log.d(DEBUG_TAG, "mFacepass handler = null");
                    continue;
                }
                ...
                try {
                    long startFeedTime = System.currentTimeMillis();
                    detectionResult = mFacePassHandler.feedFrame(image);
                    long durTime = System.currentTimeMillis() - startFeedTime;
//                    Log.d(DEBUG_TAG, "durTime is " + durTime);
                } catch (FacePassException e) {
                    e.printStackTrace();
                }

                if (detectionResult != null && detectionResult.message != null) {
                    Log.d(DEBUG_TAG, "message size " + detectionResult.message.length);
                } else {
                    Log.d(DEBUG_TAG, "message null not");
                }
